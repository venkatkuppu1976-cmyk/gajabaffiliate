  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-[#5A6378] mt-1">Live ranking, sorted by revenue generated.</p>
      </div>
            ...
            <tbody>
              {leaderboard.map(r => (
                <tr key={r.rank} className={`border-b border-[#EAE6E1] ${r.rank<=3?"bg-[#FFF6DC]":""}`} data-testid={`master-row-${r.rank}`}>
                  ...
                  <td className="p-3"><div className="flex items-center gap-2"><img src={r.avatar} alt="" className="w-8 h-8 rounded-full object-cover border border-[#EFEAE0]" /><span className="font-bold">{r.name}</span></div></td>
