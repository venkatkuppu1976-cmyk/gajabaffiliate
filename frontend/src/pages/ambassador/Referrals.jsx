  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-[#4A4A4A] mt-1">Discount codes assigned to you. Share & earn commission on every use.</p>
      </div>
          ...
          <div key={c.code} className="gajab-card p-5" data-testid={`code-${c.code}`}>
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-[#737373] mt-1">{c.type} • {c.value} off {c.cap !== "—" && `(max ${c.cap})`}</p>
              </div>
              <span className={`gajab-sticker border-2 ${c.status==="Active"?"bg-[#D1FAE5] text-[#065F46] border-[#065F46]":"bg-[#FEE2E2] text-[#991B1B] border-[#991B1B]"}`}>{c.status}</span>
            </div>
