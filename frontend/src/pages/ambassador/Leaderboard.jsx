...
const PodiumCard = ({ rank, name, college, revenue, avatar, color, height, icon: Icon }) => (
  <div className={`gajab-card p-4 text-center ${color} flex flex-col justify-end`} style={{ minHeight: height }}>
    ...
    <img src={avatar} alt="" className="w-16 h-16 mx-auto rounded-full border border-[#EFEAE0] object-cover" />
  ...
  return (
    <div className="space-y-5 pb-20">
      ...
      <div className="gajab-card p-2 sm:p-3">
        ...
        {rest.map(r => (
          <div key={r.rank} className={`grid grid-cols-12 items-center px-3 py-3 rounded-xl ${r.isYou ? "bg-[#FFF6DC] border border-[#EFEAE0]" : "hover:bg-[#FFF7EE]"}`} data-testid={`leaderboard-row-${r.rank}`}>
            ...
            <div className="col-span-7 sm:col-span-6 flex items-center gap-3 min-w-0">
              <img src={r.avatar} className="w-9 h-9 rounded-full object-cover border border-[#EFEAE0]" alt="" />
              <div className="min-w-0">
                <p className="font-bold truncate">{r.name} {r.isYou && <span className="gajab-sticker-orange text-[9px] ml-1">YOU</span>}</p>
                <p className="text-xs text-[#5A6378] sm:hidden truncate">{r.college}</p>
              </div>
            </div>
            <div className="hidden sm:block sm:col-span-3 text-sm text-[#5A6378] truncate">{r.college}</div>
