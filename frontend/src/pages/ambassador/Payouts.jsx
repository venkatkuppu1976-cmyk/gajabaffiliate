...
import { Wallet, Clock, Filter } from "lucide-react";
        ...
        <div className="space-y-2">
          {filtered.map(p => (
            <div key={p.id} className="flex items-center justify-between gap-3 p-3 rounded-xl border border-[#EFEAE0]" data-testid={`payout-${p.id}`}>
              <div><p className="font-bold text-sm">{p.period}</p><p className="text-xs text-[#5A6378]">{p.id} • {p.date}</p></div>
              <div className="text-right">
                <p className="font-display">₹{p.amount.toLocaleString()}</p>
                <span className={`text-[10px] font-extrabold uppercase tracking-wider px-2 py-0.5 rounded-full border ${p.status==="Paid"?"bg-[#D1FAE5] text-[#065F46] border-[#065F46]/40":"bg-[#FEF3C7] text-[#92400E] border-[#92400E]/40"}`}>{p.status==="Paid"?"✓":(<Clock className="w-3 h-3 inline" />)} {p.status}</span>
              </div>
            </div>
          ))}
