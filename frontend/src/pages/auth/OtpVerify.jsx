...
export default function OtpVerify() {
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2">
      <div className="hidden lg:flex bg-[#FFC93C] text-[#1A1A1A] p-12 flex-col justify-between">
        <Logo size="lg" />
        <div>
      ...
      <div className="flex items-center justify-center p-6 sm:p-12 bg-[#FDFBF7]">
        <div className="w-full max-w-md">
          ...
          <p className="text-[#4A4A4A] mt-3">Demo OTP: <b>123456</b></p>
          ...
          <div className="mt-8 flex gap-2 sm:gap-3" data-testid="otp-inputs">
            {otp.map((d, i) => (
              <input
                ...
                inputMode="numeric"
                maxLength={1}
                className="w-12 h-14 sm:w-14 sm:h-16 text-center text-2xl font-display font-extrabold rounded-xl border-2 border-black bg-white focus:outline-none focus:ring-4 focus:ring-[#FFC93C]/40"
          ...
          <p className="mt-6 text-sm text-[#4A4A4A]"><Link to="/login" className="font-bold underline">← Change number</Link> • Didn't get the code? <button className="font-bold text-[#E11D2A] underline">Resend</button></p>
        </div>
      </div>
    </div>
  );
